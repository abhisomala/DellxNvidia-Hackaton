...
3. **patch**   a Path 2-shaped report whose patch comes from the local Ollama
               model (gemma4:26b) via ``pipeline.patch_llm``; the prompt and
               raw model response are saved in the run directory.
...
from pipeline.patch_llm import build_patch  # noqa: E402
    ...
    *,
    ...
    try:
        ...
        if rule:
            ...
            if not matches:
                raise StageError(f"the scan reported no {rule!r} violation to patch")
        ...
        patch = build_patch(target_violation, source_path.read_text(), repo_root=REPO_ROOT)
        ...
        log(
            ...
            f"(model_used={patch['model_used']}, latency_s={patch['latency_s']})",
        )
...
def main() -> int:
    ...
    try:
        summary = run_pipeline(args.target, args.trigger_source, skip_report=args.skip_report, rule=args.rule)
